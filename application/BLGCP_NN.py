...
class HybridImagePCFModel(nn.Module):
    def __init__(self, pcf_channels, seq_len, img_channels, img_h, img_w, out_dim, n_aux):
        super(HybridImagePCFModel, self).__init__()
        ...
        with torch.no_grad():
            ...
            flat_feats = pcf_dim + img_dim + self.n_aux
...
def train_and_save_hybrid_NN_model(L_train, IMG_train, Y_train, AUX_train, model_path, batch_size=64, epochs=30, lr=1e-3, requested_device="auto", seed = None, verbose=True, standardize_img=True):
    ...
    model = HybridImagePCFModel(pcf_channels=pcf_channels, seq_len=seq_len, img_channels=img_channels, img_h=img_h, img_w=img_w, out_dim=n_out, n_aux=n_aux).to(device=device, dtype=DEFAULT_DTYPE)
    ...
    def run_epoch(train=True):
        ...
        for start in range(0, N, batch_size):
            ...
            del Lb, Ib, Ab, Yb, pred, pred_sigma, true_sigma, pred_scale, true_scale, loss_sigma, loss_scale, loss
...
def load_hybrid_NN_model_predict(L_test, IMG_test, AUX_test, model_path, requested_device="auto", predict_batch_size=2000):
    predict_batch_size = int(predict_batch_size)
    device = get_device(requested_device)
    load_model = torch.load(model_path, map_location=device)
    model = HybridImagePCFModel(pcf_channels=load_model["pcf_channels"], seq_len=load_model["seq_len"], img_channels=load_model["img_channels"], img_h=load_model["img_h"], img_w=load_model["img_w"], out_dim=load_model["n_out"], n_aux=load_model["n_aux"]).to(device=device, dtype=DEFAULT_DTYPE)
    ...
    with torch.no_grad():
        for start in range(0, N, predict_batch_size):
            ...
            pred = model(Lb, Ib, Ab).detach().cpu().numpy()
