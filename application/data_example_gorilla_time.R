...
R_address = "C:/Users/qihan/Desktop/data_example_time"
setwd(R_address)
...
for (i in 1:n_runs) {
  ...
  model_hybrid = train_and_save_hybrid_NN_model(
    pcf_train, img_train, Y_train, aux_train_scaled,
    model_path = sprintf("C:/Users/qihan/Desktop/data_example_time/gorilla_model_run_%d.pth", i),
    batch_size = 100,
    epochs = 30,
    lr = 1e-3,
...
t3 = Sys.time()
for (i in 1:n_runs) {
  pred_i = load_hybrid_NN_model_predict(
    pcf_obs, img_obs, aux_obs_scaled,
    sprintf("C:/Users/qihan/Desktop/data_example_time/gorilla_model_run_%d.pth", i),
    requested_device = "auto",
...
# simulated training data 3.484835 hours
# Training 10 NN 5.77782 mins
# Estimation 0.4287071 secs
...
# ------------------------------------------------------------------------------
# Generate 3 realizations from fitted model
# using estimated latent field parameters + ppm mean trend
# ------------------------------------------------------------------------------

# Estimated latent-field parameters from NN output
par_est = as.numeric(pred_mean[1, ])
names(par_est) = colnames(pred_mean)

par_est



simulate_from_fitted_model = function(par_est,
                                      fit_major,
                                      fit_minor,
                                      beta_major = NULL,
                                      beta_minor = NULL,
  ...
  sigma_Y  = par_est["sigma_Y"]
  scale_Y  = par_est["scale_Y"]
  sigma_U1 = par_est["sigma_U1"]
  sigma_U2 = par_est["sigma_U2"]
  scale_U1 = par_est["scale_U1"]
  scale_U2 = par_est["scale_U2"]
  
  # ppm mean trend for Type1 and Type2
  # If beta_major / beta_minor are NULL, this uses the fitted ppm coefficients
  lambda1 = get_lambda_trend(
    fit_major,
    beta = beta_major,
    dimyx = c(image_nxy, image_nxy)
  )
  
  lambda2 = get_lambda_trend(
    fit_minor,
    beta = beta_minor,
    dimyx = c(image_nxy, image_nxy)
  )
  ...
  for (try_id in seq_len(max_try)) {
    ...
    # log intensity mean for each type
    mu1 = eval.im(
      mean1 -
        0.5 * sigma_Y^2 -
        0.5 * sigma_U1^2 +
    ...
    # type-specific LGCP realizations
    X1 = rLGCP(
      model = "exponential",
      mu = mu1,
      var = sigma_U1^2,
    ...
    if (X1$n > 0 && X2$n > 0) {
      X_sim = superimpose(Type1 = X1, Type2 = X2, W = W)
      marks(X_sim) = factor(marks(X_sim), levels = types)
      return(X_sim)
    }
  }
  
  stop("Could not simulate non-empty Type1 and Type2 patterns.")
}
...
set.seed(2026)
sim_patterns = vector("list", 10)

for (i in 1:10) {
  sim_patterns[[i]] = simulate_from_fitted_model(
    par_est = par_est,
    fit_major = fit_major,
    fit_minor = fit_minor,
    beta_major = NULL,
...
# ------------------------------------------------------------------------------
# Function to plot one simulated realization
# ------------------------------------------------------------------------------

plot_sim_pattern = function(X_sim,
                            filename,
                            title = "Simulated gorilla nest realization",
                            plot.res = 600) {
  
  # use original gorilla window, not Window(X_sim)
  ...
  mark_sim = marks(X_sim)
  
  if (is.data.frame(mark_sim)) {
    mark_sim = mark_sim[, 1]
  }
  
  mark_sim = as.character(mark_sim)
  
  X_major_sim = unmark(X_sim[mark_sim == "Type1"])
  X_minor_sim = unmark(X_sim[mark_sim == "Type2"])
  ...
  p = ggplot() +
    ...
    scale_color_manual(
      values = c("Major" = "black", "Minor" = "red")
    ) +
    coord_equal() +
    theme_minimal()
...
plot_sim_pattern(
  sim_patterns[[1]],
  filename = "gorilla_sim_realization_1.png",
  title = "Simulated gorilla nest realisation 1",
  plot.res = plot.res
)

plot_sim_pattern(
  sim_patterns[[2]],
  filename = "gorilla_sim_realization_2.png",
  title = "Simulated gorilla nest realisation 2",
  plot.res = plot.res
)

plot_sim_pattern(
  sim_patterns[[3]],
  filename = "gorilla_sim_realization_3.png",
  title = "Simulated gorilla nest realisation 3",
  plot.res = plot.res
)
...
plot_sim_pattern(
  sim_patterns[[4]],
  filename = "gorilla_sim_realization_4.png",
  title = "Simulated gorilla nest realisation 4",
  plot.res = plot.res
)

plot_sim_pattern(
  sim_patterns[[5]],
  filename = "gorilla_sim_realization_5.png",
  title = "Simulated gorilla nest realisation 5",
  plot.res = plot.res
)

plot_sim_pattern(
  sim_patterns[[6]],
  filename = "gorilla_sim_realization_6.png",
  title = "Simulated gorilla nest realisation 6",
  plot.res = plot.res
)
...
plot_sim_pattern(
  sim_patterns[[7]],
  filename = "gorilla_sim_realization_7.png",
  title = "Simulated gorilla nest realisation 7",
  plot.res = plot.res
)

plot_sim_pattern(
  sim_patterns[[8]],
  filename = "gorilla_sim_realization_8.png",
  title = "Simulated gorilla nest realisation 8",
  plot.res = plot.res
)

plot_sim_pattern(
  sim_patterns[[9]],
  filename = "gorilla_sim_realization_9.png",
  title = "Simulated gorilla nest realisation 9",
  plot.res = plot.res
)
...
plot_sim_pattern(
  sim_patterns[[10]],
  filename = "gorilla_sim_realization_10.png",
  title = "Simulated gorilla nest realisation 10",
  plot.res = plot.res
)
...
t_e1 = Sys.time()
# ------------------------------------------------------------------------------
# Envelopes with smaller radius range
# ------------------------------------------------------------------------------

theta_hat = as.numeric(pred_mean[1, ])

# Use only the first half of the original radius range
r_small = seq(0, 0.5 * rmax, length.out = 513)

set.seed(2025)
plot.res = 600

png("gorilla_env_cross_DSBI_small_r.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
res_cross_small_r = env_mlgcp_cross_fast(
  theta = theta_hat,
  method_name = "DSBI small r",
  nsim = 99,
  r_use = r_small
)
dev.off()
...
set.seed(2025)
png("gorilla_env_major_DSBI_small_r.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
res_major_small_r = env_mlgcp_major_fast(
  theta = theta_hat,
  method_name = "DSBI small r",
  nsim = 99,
  r_use = r_small
...
set.seed(2025)
png("gorilla_env_minor_DSBI_small_r.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
res_minor_small_r = env_mlgcp_minor_fast(
  theta = theta_hat,
  method_name = "DSBI small r",
  nsim = 99,
  r_use = r_small
...
res_cross_small_r$env
res_major_small_r$env
res_minor_small_r$env

library(GET)

res_get_small_r = global_envelope_test(
  curve_sets = list(
    Cross = res_cross_small_r$env,
    Major = res_major_small_r$env,
    Minor = res_minor_small_r$env
  ),
  type = "erl"
)

res_get_small_r
plot(res_get_small_r)

png("G_env_small_r.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
plot(res_get_small_r)
dev.off()
...
# ------------------------------------------------------------------------------
# Envelopes with more simulations
# ------------------------------------------------------------------------------
t_e2 = Sys.time()
print(t_e2 - t_e1)
...
png("gorilla_env_cross_DSBI_more_sims.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
res_cross_more = env_mlgcp_cross_fast(
  theta = theta_hat,
  method_name = paste0("DSBI nsim=", nsim_more),
  nsim = nsim_more,
  r_use = r
)
dev.off()
...
set.seed(2025)
png("gorilla_env_major_DSBI_more_sims.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
res_major_more = env_mlgcp_major_fast(
  theta = theta_hat,
  method_name = paste0("DSBI nsim=", nsim_more),
  nsim = nsim_more,
  r_use = r
...
set.seed(2025)
png("gorilla_env_minor_DSBI_more_sims.png",
    width = 6.2 * plot.res, height = 4.5 * plot.res, res = plot.res)
res_minor_more = env_mlgcp_minor_fast(
  theta = theta_hat,
  method_name = paste0("DSBI nsim=", nsim_more),
  nsim = nsim_more,
  r_use = r
...
res_get_more = global_envelope_test(
  curve_sets = list(
    Cross = res_cross_more$env,
    Major = res_major_more$env,
    Minor = res_minor_more$env
  ),
  type = "erl"
)
