...
extract_one_case_features = function(params, r, image_nxy_set = NULL) {
  ...
  for (a in 1:(length(types) - 1)) {
    for (b in (a + 1):length(types)) {
      ...
      if (is.null(gc)) return(list(ok = FALSE))
  ...
  for (i in types) {
    ...
    gi = tryCatch(
      ...
      error = function(e) NULL)
    if (is.null(gi)) return(list(ok = FALSE))
  ...
  list(ok = TRUE, pcf = pcf_within_list, pcfcross = pcf_cross_list, beta = beta_vec, count_img = count_img)
}
...
build_param_list = function(idx, beta0_1, beta0_2, beta1_1, beta1_2, beta2_1, beta2_2, sigma_y, sigma_u_1, sigma_u_2, scale_y, scale_u_1, scale_u_2, Z1, Z2) {
  list(p = 2, L = 1.5, nx = 50, ny = 50, beta0 = c(beta0_1[idx], beta0_2[idx]), beta1 = c(beta1_1[idx], beta1_2[idx]), beta2 = c(beta2_1[idx], beta2_2[idx]), Y_var = sigma_y[idx], U_var = c(sigma_u_1[idx], sigma_u_2[idx]), Y_scale = scale_y[idx], U_scale = c(scale_u_1[idx], scale_u_2[idx]), Z_field1 = Z1, Z_field2 = Z2)
}
...
stack_feature_results = function(res_list, response_par = NULL) {
  ...
  aux_input = beta_mat
  out = list(keep_true = keep_true, keep_pcf = keep_pcf, pcf_input = pcf_input, count_img = count_img, aux_input = aux_input, beta_input = beta_mat)
...
# Covariates
set.seed(123)
Z1 = simulate_grf(50, 50, 1.5, range = 0.01, var = 1)
Z1 = Z1 - mean(Z1)
set.seed(456)
Z2 = simulate_grf(50, 50, 1.5, range = 0.01, var = 1)
Z2 = Z2 - mean(Z2)

# png("BLGCP covariate field Z1(s).png", width = 800, height = 800)  
# plot(as.im(Z1), main = "BLGCP covariate Z1(s)") 
# dev.off()  
# png("BLGCP covariate field Z2(s).png", width = 800, height = 800)  
# plot(as.im(Z2), main = "BLGCP covariate Z2(s)") 
# dev.off()  
...
save.image("C:/Users/qihan/Desktop/DSBI_bivariateLGCP/33sim2_training.RData")
# ------------------------------------------------------------------------------
# Test
ntest = 5000
#set.seed(2025)
...
for (i in 1:n_runs) {
  ...
  model_path_i = sprintf("C:/Users/qihan/Desktop/DSBI_bivariateLGCP/33model_50_run_%d.pth", i)
...
save.image("C:/Users/qihan/Desktop/DSBI_bivariateLGCP/sim2_33.RData")
...
pred_par_50_w2 = pred_par_50
test_par_w2 = test_par



# ------------------------------------------------------------------------------
# Prediction error boxplots: compare diff_50 and diff_50_w2
# Sigma parameters together and scale parameters together
# ------------------------------------------------------------------------------

diff_50    = pred_par_50 - test_par
diff_50_w2 = pred_par_50_w2 - test_par_w2

par_names = c("sigma_Y", "scale_Y", "sigma_U1", "sigma_U2", "scale_U1", "scale_U2")

colnames(diff_50) = par_names
colnames(diff_50_w2) = par_names

diff_df = rbind(
  data.frame(Method = "W1", diff_50),
  data.frame(Method = "W2", diff_50_w2)
)
...
plot_order = c("sigma_Y", "sigma_U1", "sigma_U2",
               "scale_Y", "scale_U1", "scale_U2")

diff_long$Parameter = factor(diff_long$Parameter, levels = plot_order)
diff_long$Group = factor(diff_long$Group, levels = c("sigma", "scale"))
diff_long$Method = factor(diff_long$Method, levels = c("W1", "W2"))

library(ggplot2)

p_diff_compare = ggplot(diff_long, aes(x = Parameter, y = Error, fill = Method)) +
  geom_boxplot(
    outlier.size = 0.6,
    colour = "black",
  ...
  theme_bw() +
  labs(
    title = bquote("Prediction error boxplots for two windows: " ~
                     W[1] == "[0,1]"^2 * " and " ~
                     W[2] == "[0,1.5]"^2),
    x = "Parameter",
    y = "Prediction error",
...
png(filename = "inhom_boxplot_compare_windows.png", width = 2600, height = 1800, res = 300)
print(p_diff_compare)
dev.off()
