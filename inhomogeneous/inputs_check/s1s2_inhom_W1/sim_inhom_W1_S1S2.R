...
R_address = "C:/Users/qihan/Desktop/sim_inhom_S1_S2"
setwd(R_address)
...
extract_one_case_features = function(params, r) {
  W = owin(c(0, 1), c(0, 1))
  types = c("Type1", "Type2")
  ...
  list(ok = TRUE, pcf = pcf_within_list, pcfcross = pcf_cross_list, beta = beta_vec)
}
...
stack_feature_results = function(res_list, response_par = NULL) {
  ...
  aux_input = beta_mat
  out = list(keep_true = keep_true, keep_pcf = keep_pcf, pcf_input = pcf_input, aux_input = aux_input, beta_input = beta_mat)
...
with_progress({
  ...
  res_train = future_lapply(
    ...
    function(j) {
      ...
      extract_one_case_features(params, r)
    },
    future.seed = 2026,
...
save.image("C:/Users/qihan/Desktop/sim_inhom_S1_S2/sim2_training_W1_S1_S3.RData")
# ------------------------------------------------------------------------------
# Test
ntest = 5000
set.seed(2027)
...
# ------------------------------------------------------------------------------
# DSBI 
source_python("BLGCP_NN_S1S2.py")
...
for (i in 1:n_runs) {
  ...
  model_path_i = sprintf("C:/Users/qihan/Desktop/sim_inhom_S1_S2/W1_model_beta_L_only_run_%d.pth", i)
  model_hybrid = train_and_save_hybrid_NN_model(pcf_train_scaled, Y_train, aux_train_scaled, model_path = model_path_i, batch_size = 100, epochs = 30, lr = 1e-3, requested_device = "auto", seed = seed_i, verbose = TRUE)
  pred_par = load_hybrid_NN_model_predict(pcf_test_scaled, aux_test_scaled, model_path_i, requested_device = "auto", predict_batch_size = predict_batch_size)^2
...
save.image("C:/Users/qihan/Desktop/sim_inhom_S1_S2/sim2_W1_S1_S3.RData")
...
diff_df = data.frame(Method = "Beta + L only", diff_50)
