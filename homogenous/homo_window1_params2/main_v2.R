...
# Homogeneous training data from simulation study 1
#load("C:/Users/qihan/Desktop/DSBI_BivariateLGCP/sim1_training.RData")
...
build_param_list = function(idx, beta0_1, beta0_2, sigma_y, sigma_u_1, sigma_u_2, scale_y, scale_u_1, scale_u_2) {
  list(p = 2, L = 1, nx = 50, ny = 50, beta0 = c(beta0_1[idx], beta0_2[idx]), Y_var = sigma_y[idx], U_var = c(sigma_u_1[idx], sigma_u_2[idx]), Y_scale = scale_y[idx], U_scale = c(scale_u_1[idx], scale_u_2[idx]))
}
...
stack_feature_results = function(res_list, response_par = NULL) {
  ...
  aux_input = beta_mat
  out = list(keep_true = keep_true, keep_pcf = keep_pcf, pcf_input = pcf_input, count_img = count_img, aux_input = aux_input, beta_input = beta_mat, Np = Np)
  out$X = X_ok[keep_pcf]
...
par_ini_QBMC = c(1.5, 0.12, 0.7, 0.1, 0.8, 0.1)
types = c("Type1", "Type2")
image_nxy_set = c(50, 10)
# ------------------------------------------------------------------------------
# Test
ntest = 500
set.seed(2026)
...
scale_y_test = runif(ntest, 0.12, 0.12)
sigma_u_1_test = runif(ntest, 0.7, 0.7)
sigma_u_2_test = runif(ntest, 0.8, 0.8)
scale_u_1_test = runif(ntest, 0.1, 0.1)
scale_u_2_test = runif(ntest, 0.1, 0.1)
...
QBMC_rmax_grid = seq(0.05, 0.25, by = 0.025)
...
for (i in 1:n_runs) {
  ...
  model_path_i = sprintf("C:/Users/qihan/Desktop/DSBI_bivariateLGCP/model_50_10_run_%d.pth", i)
  model_hybrid = train_and_save_hybrid_NN_model(pcf_train_scaled, img_train, Y_train, aux_train_scaled, model_path = model_path_i, batch_size = 100, epochs = 30, lr = 1e-3, requested_device = "auto", seed = seed_i, verbose = TRUE, standardize_img = TRUE)
  pred_par = load_hybrid_NN_model_predict(pcf_test_scaled, img_test, aux_test_scaled, model_path_i, requested_device = "auto", predict_batch_size = predict_batch_size)^2
  res_all[[i]] = pred_par
  rm(model_hybrid, pred_par)
  clear_all()
  gc()
}
pred_par = get_mean_sd(res_all)$mean
png(filename = "dsbi_50_10.png", width = 2400, height = 1800, res = 300)
plot_all_par(test_par, pred_par, main_prefix = "DSBI")
dev.off()
compare_hybrid = compare_par(test_par, pred_par, c("sigma_Y", "scale_Y", "sigma_U1", "sigma_U2", "scale_U1", "scale_U2"))
compare_hybrid[] = lapply(compare_hybrid, function(x) if(is.numeric(x)) sprintf("%.3f", x) else x)
print(compare_hybrid, row.names = FALSE)


# ------------------------------------------------------------------------------
# DSBI: 50x50 images only
...
for (i in 1:n_runs) {
  ...
  seed_i = sample.int(10000, 1)
  set_all_seeds(seed_i)
  model_path_i = sprintf("C:/Users/qihan/Desktop/DSBI_bivariateLGCP/model_50_run_%d.pth", i)
...
pred_mean_QBMC = matrix(NA, nrow = ntest, ncol = 2)
colnames(pred_mean_QBMC) = c("lambda1_hat", "lambda2_hat")
pred_mean_QBMC[keep_QBMC, ] = do.call(rbind, lapply(res_test[keep_QBMC], function(x) {
  x$Est_MC[7:8]
}))

true_mean_QBMC = matrix(NA, nrow = ntest, ncol = 2)
colnames(true_mean_QBMC) = c("lambda1_true", "lambda2_true")
true_mean_QBMC[, 1] = beta0_1_test
true_mean_QBMC[, 2] = beta0_2_test
compare_mean_QBMC = compare_par(true_mean_QBMC[keep_QBMC, , drop = FALSE], pred_mean_QBMC[keep_QBMC, , drop = FALSE], c("lambda1", "lambda2"))
...
png(
  filename = "sim2_boxplot_all.png",
  width = 2400,
  height = 1800,
  res = 300
)

param_names = c("sigma_Y", "sigma_U1", "sigma_U2",
...
true_values = c(1.5, 0.7, 0.8,
                0.12, 0.1, 0.1)

par_order = c(1, 3, 4, 2, 5, 6)
...
for (kk in 1:6) {
  ...
  boxplot(
    pred_par[, k],
    pred_par_50[, k],
    pred_par_QBMC[keep_QBMC, , drop = FALSE][idx, , drop = FALSE][, k],
    names = c("50+10", "50", "QMC"),
    main = param_names[kk],
    ylim = ylim_list[[kk]]
  )
  
  abline(h = true_values[kk], lty = 2, lwd = 1.5, col = "red")
}

dev.off()


save.image("C:/Users/qihan/Desktop/DSBI_BivariateLGCP/sim1.RData")
...
INLA_res = with_progress({
  ...
  lapply(seq_along(X_inla_list), function(j) {
...
save.image("C:/Users/qihan/Desktop/DSBI_BivariateLGCP/sim2_inla.RData")
...
png(
  filename = "sim1_boxplot_param1_W1.png",
  width = 2800,
  height = 1800,
  res = 300
)
...
for (kk in 1:6) {
  ...
  boxplot(
    pred_par_50[, k],
    pred_par_QBMC[keep_QBMC, , drop = FALSE][idx, , drop = FALSE][, k],
    pred_par_INLA[keep_INLA, , drop = FALSE][idx_INLA, , drop = FALSE][, k],
...
inla_data = pred_par_INLA[keep_INLA, , drop = FALSE][idx_INLA, , drop = FALSE]
inla_data[,2] = inla_data[,2]/2
inla_data[,5] = inla_data[,5]/2
inla_data[,6] = inla_data[,6]/2


compare_INLA1 = compare_par(test_par_full[keep_INLA, , drop = FALSE][idx_INLA, , drop = FALSE], inla_data, c("sigma_Y", "scale_Y", "sigma_U1", "sigma_U2", "scale_U1", "scale_U2"))
compare_INLA1[] = lapply(compare_INLA1, function(x) if (is.numeric(x)) sprintf("%.3f", x) else x)
print(compare_INLA1, row.names = FALSE)
...
png(
  filename = "sim1_boxplot_param1_W1_v2.png",
  width = 2800,
  height = 1800,
  res = 300
)
...
######################
# ============================================================
# INLA per-parameter filtering using training ranges
# ============================================================
...
colnames(pred_par_50)   <- par_names
colnames(pred_par_QBMC) <- par_names
colnames(pred_par_INLA) <- par_names
colnames(test_par_full) <- par_names
colnames(test_par)      <- par_names

lower_train <- lower_train[par_names]
upper_train <- upper_train[par_names]


# ============================================================
# Per-parameter INLA filtering
# ============================================================

pred_par_INLA_pp <- matrix(
  NA,
  nrow = nrow(pred_par_INLA),
  ncol = ncol(pred_par_INLA)
)

true_par_INLA_pp <- matrix(
  NA,
  nrow = nrow(test_par_full),
  ncol = ncol(test_par_full)
)

colnames(pred_par_INLA_pp) <- par_names
colnames(true_par_INLA_pp) <- par_names

for (k in seq_along(par_names)) {
  par_k <- par_names[k]
  
  idx_k <- keep_INLA &
    is.finite(pred_par_INLA[, k]) &
    pred_par_INLA[, k] >= lower_train[par_k] &
    pred_par_INLA[, k] <= upper_train[par_k]
  
  pred_par_INLA_pp[idx_k, k] <- pred_par_INLA[idx_k, k]
  true_par_INLA_pp[idx_k, k] <- test_par_full[idx_k, k]
}


# ============================================================
# Compare INLA after per-parameter filtering
# ============================================================

compare_INLA_pp <- compare_par(
  true_par_INLA_pp,
  pred_par_INLA_pp,
  par_names
)

compare_INLA_pp_print <- compare_INLA_pp

compare_INLA_pp_print[] <- lapply(
  compare_INLA_pp_print,
  function(x) if (is.numeric(x)) sprintf("%.3f", x) else x
)

print(compare_INLA_pp_print, row.names = FALSE)


# ============================================================
# Boxplot with per-parameter INLA filtering
# ============================================================

png(
  filename = "sim2_boxplot_param_INLA_filtered.png",
  width = 2800,
  height = 1800,
  res = 300
)
...
pred_QBMC_plot <- pred_par_QBMC[keep_QBMC, , drop = FALSE][idx, , drop = FALSE]
...
for (kk in 1:6) {
  ...
  boxplot(
    pred_par_50[, k],
    pred_QBMC_plot[, k],
    pred_par_INLA[idx_INLA_k, k],
    names = c("DSBI", "MC", "INLA"),
    main = param_names_plot[kk],
    ylim = ylim_list[[kk]]
  )
