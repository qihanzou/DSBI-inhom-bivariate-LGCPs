...
library(sf)
library(fmesher)
source("C:/Users/qihan/Desktop/hom_W1_time/MC.R")
source("C:/Users/qihan/Desktop/hom_W1_time/QMC.R")
...
extract_one_case_combined = function(params, r, par_ini_BMC = NULL, use_BMC = FALSE,
                                     image_nxy_set = NULL,
                                     par_ini_QBMC, QBMC_power, QBMC_rmax,
                                     types = c("Type1", "Type2"), Corr = "Pos",
                                     run_QBMC = TRUE) {
  W = owin(c(0, 1), c(0, 1))
...
fit_QBMC_only = function(X, params, par_ini_QBMC, QBMC_power, QBMC_rmax, Corr = "Pos") {
  
  t0_QBMC_case = Sys.time()
  ...
  t1_QBMC_case = Sys.time()
  
  qmc_time_min = as.numeric(
    difftime(t1_QBMC_case, t0_QBMC_case, units = "mins")
  )
...
with_progress({
  ...
  res_test = future_lapply(
    seq_len(ntest),
    function(j) {
      p(sprintf("test data iteration %d/%d", j, ntest))
      ...
      extract_one_case_combined(
        params,
        r,
        par_ini_BMC = par_ini_BMC,
        use_BMC = TRUE,
        image_nxy_set = image_nxy_set,
        par_ini_QBMC = par_ini_QBMC,
        QBMC_power = QBMC_power_opt,
        QBMC_rmax = QBMC_rmax_opt,
        types = types,
        Corr = Corr,
        run_QBMC = FALSE
      )
...
t_test1 = Sys.time()
print(t_test1 - t_test0)



# 500 testing data, 1.306 mins.
...
# ------------------------------------------------------------------------------
# DSBI 
t_NN_train0 = Sys.time()

img_train_50 = img_train[, , , 1:3, drop = FALSE]
img_test_50  = img_test[, , , 1:3, drop = FALSE]

# only first testing data
pcf_test_scaled_1 = pcf_test_scaled[1, , , drop = FALSE]
img_test_50_1     = img_test_50[1, , , , drop = FALSE]
aux_test_scaled_1 = aux_test_scaled[1, , drop = FALSE]
test_par_1        = test_par[1, , drop = FALSE]
...
for (i in 1:n_runs) {
  ...
  model_path_i = sprintf(
    "C:/Users/qihan/Desktop/hom_W1_time/W1_sim1_model_50_run_%d.pth",
    i
  )
...
t_NN_train1 = Sys.time()

print(t_NN_train1 - t_NN_train0)


# 56.09037 mins 100000
# 6.317498 mins 10000
...
for (i in 1:n_runs) {
  ...
  pred_par = load_hybrid_NN_model_predict(
    pcf_test_scaled_1,
    img_test_50_1,
    aux_test_scaled_1,
    model_path_i,
...
print(t_NN_pred1 - t_NN_pred0)

# 3.368695 secs 100000
# 2.480179 secs 10000
...
INLA_res = with_progress({
  p_INLA = progressor(along = seq_len(1))
  future_lapply(
    seq_len(1),
    function(j) {
      p_INLA(sprintf("INLA iteration %d/%d", j, 1))
