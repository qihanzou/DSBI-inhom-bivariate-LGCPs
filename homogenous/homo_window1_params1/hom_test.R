...
library(sf)
library(fmesher)
source("C:/Users/qihan/Desktop/hom_W1/MC.R")
source("C:/Users/qihan/Desktop/hom_W1/QMC.R")
...
for (i in 1:n_runs) {
  ...
  model_path_i = sprintf("C:/Users/qihan/Desktop/hom_W1/W1_sim1_model_50_run_%d.pth", i)
...
save.image("C:/Users/qihan/Desktop/hom_W1/sim1.RData")
...
save(
  useful_output,
  file = "C:/Users/qihan/Desktop/hom_W1/sim1_useful_output.RData"
)



load("C:/Users/qihan/Desktop/hom_W1/sim1_useful_output.RData")

pred_par_50   <- useful_output$predictions$DSBI_50
pred_par_QBMC <- useful_output$predictions$QBMC
pred_par_INLA <- useful_output$predictions$INLA

test_par_full <- useful_output$true_parameters$test_par_full
test_par      <- useful_output$true_parameters$test_par

keep_QBMC <- useful_output$valid_indices$keep_QBMC
idx       <- useful_output$valid_indices$idx_QBMC

keep_INLA <- useful_output$valid_indices$keep_INLA
idx_INLA  <- useful_output$valid_indices$idx_INLA
...
compare_INLA <- compare_par(
  true_par_INLA_pp,
  pred_par_INLA_pp,
  par_names
)

compare_INLA_print <- compare_INLA

compare_INLA_print[] <- lapply(
  compare_INLA_print,
  function(x) if (is.numeric(x)) sprintf("%.3f", x) else x
)

print(compare_INLA_print, row.names = FALSE)



png(
  filename = "sim1_boxplot_param2_W1_final.png",
  width = 2800,
  height = 1800,
  res = 300
)
...
compare_hybrid_50 <- compare_par(
  test_par,
  pred_par_50,
  par_names
)

compare_hybrid_50_print <- compare_hybrid_50

compare_hybrid_50_print[] <- lapply(
  compare_hybrid_50_print,
  function(x) if (is.numeric(x)) sprintf("%.3f", x) else x
)

print(compare_hybrid_50_print, row.names = FALSE)



# QMC

load("C:/Users/qihan/Desktop/hom_W1/sim1_useful_output_W1.RData")
...
compare_QBMC <- compare_par(
  test_par_full[keep_QBMC, , drop = FALSE][idx, , drop = FALSE],
  pred_par_QBMC[keep_QBMC, , drop = FALSE][idx, , drop = FALSE],
  par_names
)
